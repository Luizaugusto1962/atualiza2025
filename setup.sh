...
# .atualizac e .atualizap, que são essenciais para o funcionamento do sistema.
#
# Modos de Operação:
...
_initial_setup() {
    ...
    echo "###      ( Parâmetros para serem usados no atualiza.sh )          ###" >> .atualizap
    echo "$traco" >> .atualizap
    ...
    # Criar script de lote se offline
...
_setup_diretorios() {
    read -rp "Informe o diretório raiz sem o /: " destino
    echo "destino=${destino}" >> .atualizac

    read -rp "Nome da pasta da base (Ex: sav/dados): " base
...
_setup_acesso_remoto() {
    read -rp "Ativar acesso fácil (SSH) [S/N]: " -n1 acessossh
    echo
...
_setup_backup() {
    read -rp "Caminho de envio do backup no servidor (Ex: cliente/NOME_CLIENTE): " ENVIABACK
    if [[ -z "$ENVIABACK" && "${SERACESOFF}" =~ ^[Nn]$ ]]; then
        echo "ENVIABACK=" >> .atualizac
    elif [[ -n "$ENVIABACK" ]]; then
        echo "ENVIABACK=${ENVIABACK}" >> .atualizac
    else
...
main() {
    ...
    if [[ "$1" == "--edit" ]]; then
        ...
        if [[ -f "cfg/.atualizac" && -f "cfg/.atualizap" ]]; then
